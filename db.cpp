...
void changeGoods(const std::string& g_name,uint64_t uid) {
    query.exec(QString("update goods set origin_price = %1 where name = '").arg(goods_list[std::pair(g_name,uid)]->getPrice())+QString::fromStdString(g_name)+QString("'"));
    query.exec(QString("update goods set remain = %1 where name = '").arg(goods_list[std::pair(g_name,uid)]->getRemain())+QString::fromStdString(g_name)+QString("'"));
}
...
void deleteGoods(const std::string& g_name,uint64_t uid) {
    ...
    query.exec(QString("delete from goods where name = '")+QString::fromStdString(g_name)+QString("'"));
...
void loadGoods() {
    ...
    while (query.next()) {
        ...
        switch (g_type) {
            case 0:
                g=new Food(g_price,g_name,g_desc,g_remain);
                goods_list[pnu]=g;
                break;
            case 1:
                g=new Cloths(g_price,g_name,g_desc,g_remain);
                goods_list[pnu]=g;
                break;
            case 2:
                g=new Book(g_price,g_name,g_desc,g_remain);
                goods_list[pnu]=g;
                break;
            case 3:
                g=new Electronic(g_price,g_name,g_desc,g_remain);
                goods_list[pnu]=g;
                break;
            default:
                break;
